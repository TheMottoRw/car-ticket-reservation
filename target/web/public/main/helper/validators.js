function validatePassword(pwd) {
    var res = { "status": false, "message": `Password must be 6 characters containing lowercase,uppercase letter,numbers and at least one special character` };
...
function validatePhoneNumber(number){
    var res = { "status": false, "message": `Invalid phone number` };
    const pattern = /^\+?(?:[0-9] ?){8,14}[0-9]$/;
    if(pattern.test(number)){
        res.status = true;
        res.message='valid';
    }
